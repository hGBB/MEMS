...
def collector(interval, uuid, name):
    while True:
        with open('/home/team1/output.log', 'a') as logfile:
            network = net.get_net_io()
            energy = power.get_power()
            kpi_package['uuid'] = uuid
            kpi_package['node_name'] = name
            kpi_package['interval'] = str(interval)
            kpi_package['kpis'][0]['value'] = str(cpu.get_cpu())
            kpi_package['kpis'][1]['value'] = str(disk.get_disk_usage())
            kpi_package['kpis'][2]['value'] = str(mem.get_virt_mem())
            kpi_package['kpis'][3]['value'] = str(network[0])
            kpi_package['kpis'][4]['value'] = str(network[1])
            kpi_package['kpis'][5]['value'] = energy[0]
            kpi_package['kpis'][6]['value'] = energy[1]
            kpi_package['kpis'][7]['value'] = energy[2]
            logfile.write(str(kpi_package) + '\n')
            logfile.close()
        time.sleep(interval)
...
if __name__ == '__main__':
    ...
    p = multiprocessing.Process(target=collector, args=(args.interval, uuid, args.name,))
    p.name = 'collector.py'
    p.start()
    ...
    while True:
        try:
            ...
            if change != process_data['interval']:
                ...
                p = multiprocessing.Process(target=collector, args=(change, uuid, args.name))
                p.start()
                process_data['pid'] = p.pid
                process_data['process_instance'] = p
